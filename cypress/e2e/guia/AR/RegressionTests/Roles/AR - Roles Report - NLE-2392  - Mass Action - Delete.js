...
let secondRole = "";
describe('C1939 AUT-498 - GUIA-Story - NLE-2392 - Roles Report - Mass Action - Delete', () => {
    ...
    it('Verifying that selecting multiple roles Delete Role Button becomes available', () => {
        ...
        cy.get(ARCoursesPage.getCoursesActionsButtonsByLabel('Delete Roles')).should('have.attr', 'aria-disabled', 'false').click()
        //Asserting that modal header is delete role
        cy.get(ARDeleteModal.getModalHeader()).should('have.text', 'Delete Roles')
        //Asserting Prompt content 
        cy.get(ARDeleteModal.getDeletePromptContent()).should('contain', `Are you sure you want to delete these ${numberOfRoles} roles?`)
        ...
        //Selection prevails
        cy.get(ARDashboardPage.getTableCellRecord()).contains(secondRole).parent().find('input').first().should('have.attr', 'aria-checked', 'true')
        //Clicking on Delete Roles button
        cy.get(ARCoursesPage.getCoursesActionsButtonsByLabel('Delete Roles')).should('have.attr', 'aria-disabled', 'false').click()
        //Asserting that modal header is delete role
        cy.get(ARDeleteModal.getModalHeader()).should('have.text', 'Delete Roles')
        ...
        cy.get(ARDashboardPage.getToastNotificationMsg(), { timeout: 150000 }).should('contain', 'Roles have been deleted.')
    ...
    it(`Verify that Roles are not present in ${userDetails.username}`, () => {
        ...
        cy.get(ARUserAddEditPage.getRolesByNameInsideSelected(secondRole), { timeout: 150000 }).should('not.exist')
    })
    // 
