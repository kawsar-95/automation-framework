...
describe('AR - CED - User - Admin', () => {
    ...
    it('Verify an Admin can Create an Admin Type User', () => {
        //Add new user
        ...
        for (let i = 0; i < adminRoles.allRoles.length; i++) {
            cy.get(ARUserAddEditPage.getRolesDDownSearchTxtF()).type(adminRoles.allRoles[i]) 
            ARUserAddEditPage.getLShortWait()
            cy.get(ARUserAddEditPage.getRolesDDownOpt()).contains(adminRoles.allRoles[i]).click('bottom', { force: true })
            cy.get(ARUserAddEditPage.getRolesDDownSearchTxtF()).type('{selectall}{backspace}')
            cy.log("Next role is: " + adminRoles.allRoles[i])
        }
        cy.get(ARUserAddEditPage.getRolesDDown()).click() //close ddown

        //Verify permissions modal
        cy.get(ARUserAddEditPage.getPermissionsBtn()).click()
        ARUserAddEditPage.getLongWait
        cy.get(ARPermissionsModal.getModelHeader()).should('contain', 'Permission Overview')
        ...
        cy.get(ARUserAddEditPage.getCountryDDownSearchInputTxt()).type(ecommFields.country)
        ...
        cy.get(ARUserAddEditPage.getSupervisorDDownOpt()).contains(employmentDetailsSectionFields.supervisor).click()
        ...
        cy.get(ARUserAddEditPage.getLanguageDDownOpt()).contains(detailsSectionFields.language).click('bottom', { force: true })
    ...
    it('Verify Admin Type User Creation Persists, Edit Admin', () => {
        ...
        cy.get(ARUserAddEditPage.getElementByDataNameAttribute(ARUserAddEditPage.getAdminToggleContainer()) + ' ' + ARUserAddEditPage.getToggleStatus())
            .should('have.attr', 'aria-checked', 'true')

        //Verify user management and all admin roles persisted
        for (let i = 0; i < adminRoles.allRoles.length; i++) {
            cy.get(ARUserAddEditPage.getRolesDDown()).should('contain.text', adminRoles.allRoles[i])
        }

        //Switch to Department management
        cy.get(ARUserAddEditPage.getUserManagementRadioBtn()).contains('Department').click()
        //Verify department selection is mandatory
        cy.get(ARUserAddEditPage.getErrorMsg()).should('contain', 'Each Rule Must Have A Selected Department')
        ...
        cy.get(ARUserAddEditPage.getCountryDDown()).click()
        cy.get(ARUserAddEditPage.getCountryDDownSearchInputTxt()).type(ecommFields.country2)
        cy.get(ARUserAddEditPage.getCountryDDownOpt()).contains(ecommFields.country2).click()
        cy.get(ARUserAddEditPage.getStateProvinceDDown()).click()
        cy.get(ARUserAddEditPage.getStateProvinceDDownSearchInputTxt()).type(ecommFields.province2)
    ...
    it('Verify Admin Type Edits Persist', () => {
        ...
        //Verify Department management persisted
        cy.get(ARUserAddEditPage.getManageDepartmentTxtF(1)).should('have.value', `.../${departments.Dept_B_name}`)

        //Switch to group management
        cy.get(ARUserAddEditPage.getUserManagementRadioBtn()).contains('Group').click()
        cy.get(ARUserAddEditPage.getGroupsDDown()).click()
        cy.get(ARUserAddEditPage.getGroupsDDownSearchTxtF()).type(users.groups.guia_group_name)
        cy.get(ARUserAddEditPage.getGroupsDDownOpt()).contains(users.groups.guia_group_name).click()
        ...
        cy.get(ARUserAddEditPage.getCountryDDown()).should('contain.text', ecommFields.country2)
        cy.get(ARUserAddEditPage.getStateProvinceDDown()).should('contain.text', ecommFields.province2)
    ...
    it('Verify an Admin Type User can be Deleted', () => {
