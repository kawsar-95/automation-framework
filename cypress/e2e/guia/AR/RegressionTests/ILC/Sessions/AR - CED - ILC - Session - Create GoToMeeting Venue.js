...
describe('AR - CED - ILC - Session - Create GoToMeeting Venue', function(){
    ...
    it('Verify GoToMeeting Venue can be Created when Adding Session to ILC', () => {
        ...
        cy.get(ARILCAddEditPage.getVenueTypeDDownOpt()).contains(venueTypes.goToMeeting).click()

        //Enter valid name
        cy.get(ARILCAddEditPage.getAddVenueNameTxtF()).type(`${venueDetails.venueName} - ${venueTypes.goToMeeting}`)
    ...
    it('Verify Venue Details Persist, Delete Venue', () => {
        ...
        cy.get(ARVenueAddEditPage.getNameTxtF()).should('have.value', `${venueDetails.venueName} - ${venueTypes.goToMeeting}`)
