...
describe('AR - CED - Venues - Connect Pro', function(){
    ...
    it(`Verify ${venueTypes.connectPro} Venue can be Created`, () => {
        ...
        cy.get(ARVenueAddEditPage.getTypeDDownOpt()).contains(venueTypes.connectPro).click()
        ...
        //Enter valid name
        cy.get(ARVenueAddEditPage.getNameTxtF()).type(`${venueDetails.venueName} - ${venueTypes.connectPro}`)
        ...
        cy.get(ARVenueAddEditPage.getUsernameTxtF()).type(userDetails.username)
        cy.get(ARVenueAddEditPage.getPasswordTxtF()).type(userDetails.validPassword)
        //Verify Url is required
        cy.get(ARVenueAddEditPage.getUrlTxtF()).type('a').clear()
        cy.get(ARVenueAddEditPage.getErrorMsg()).should('contain', miscData.field_required_error)
        cy.get(ARVenueAddEditPage.getSaveBtn()).should('have.attr', 'aria-disabled', 'true')
        //Enter valid Url
        cy.get(ARVenueAddEditPage.getUrlTxtF()).type(miscData.switching_to_absorb_img_url)
    ...
    it(`Verify ${venueTypes.connectPro} Venue Values Persist, Edit Venue`, () => {
        ...
        cy.get(ARVenueAddEditPage.getNameTxtF()).should('have.value', `${venueDetails.venueName} - ${venueTypes.connectPro}`).type(commonDetails.appendText)
        ...
        cy.get(ARVenueAddEditPage.getPhoneTxtF()).should('have.value', venueDetails.phoneNumber)
            .clear().type(venueDetails.phoneNumber2)
        cy.get(ARVenueAddEditPage.getUsernameTxtF()).should('have.value', userDetails.username)
                .clear().type(users.learner01.learner_01_username)
        cy.get(ARVenueAddEditPage.getPasswordTxtF()).should('have.value', userDetails.validPassword).type(commonDetails.appendText)
        cy.get(ARVenueAddEditPage.getUrlTxtF()).should('have.value', miscData.switching_to_absorb_img_url)
            .clear().type(miscData.remote_vide0_url)
    ...
    it(`Verify ${venueTypes.connectPro} Venue Edits Persist`, () => {
        //Filter for and edit venue
        ARVenueAddEditPage.AddFilter('Name', 'Contains', `${venueDetails.venueName} - ${venueTypes.connectPro}${commonDetails.appendText}`)
        ARVenueAddEditPage.selectTableCellRecord(`${venueDetails.venueName} - ${venueTypes.connectPro}${commonDetails.appendText}`)
        ARVenueAddEditPage.WaitForElementStateToChange(ARVenueAddEditPage.getAddEditMenuActionsByName('Edit Venue'))
        cy.get(ARVenueAddEditPage.getAddEditMenuActionsByName('Edit Venue')).click()
        ARVenueAddEditPage.getLShortWait()

        //Verify values persisted
        cy.get(ARVenueAddEditPage.getNameTxtF()).should('have.value', `${venueDetails.venueName} - ${venueTypes.connectPro}${commonDetails.appendText}`)
        ...
        //Verify specific fields
        cy.get(ARVenueAddEditPage.getPhoneTxtF()).should('have.value', venueDetails.phoneNumber2)
        cy.get(ARVenueAddEditPage.getUsernameTxtF()).should('have.value', users.learner01.learner_01_username)
        cy.get(ARVenueAddEditPage.getPasswordTxtF()).should('have.value', userDetails.validPassword+commonDetails.appendText)
        cy.get(ARVenueAddEditPage.getUrlTxtF()).should('have.value', miscData.remote_vide0_url)
    })

    it(`Verify ${venueTypes.connectPro} Venue Can Be Deleted`, () => {
