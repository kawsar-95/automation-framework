...
describe('C1750 AUT-401, AR - CURR - Curriculum Expiry Handling - Course Details (Front-End)', function(){ 
    before(function() {
    ...
    it('Edit Curriculum, remove Expiration & Publish Course', () => {
        cy.apiLoginWithSession(users.sysAdmin.admin_sys_01_username, users.sysAdmin.admin_sys_01_password, '/admin')
        ARDashboardPage.getCoursesReport()
        ...
        //Select Expiration 'No Expiration' Option
        cy.get(ARCourseSettingsAvailabilityModule.getExpirationRadioBtn()).contains('No Expiration').click().click()
        ARCourseSettingsAvailabilityModule.verifyExpirationRadioBtnByLabel('No Expiration')

        //Publish Curriculum
        cy.publishCourse()
    })

    it('Login as learner 1 and Verify the expiry date on Curriculum', () => {
        ...
        // Expire Date does not exist
        cy.get(LECourseDetailsCurrModule.getCourseExpiryMessage()).should('not.exist')
        cy.get(LECourseDetailsCurrModule.getCourseExpiryDateMessage()).should('not.exist')
        ...
        // Expire Date does not exist
        cy.get(LECourseDetailsOCModule.getWarningLabel()).should('not.exist')
    ...
    it('Login as learner 2 and Verify the expiry date on Online Course', () => {
        ...
        // verify Expire Date
        LECourseDetailsOCModule.verifyExpirationDate(LEDashboardPage.getFutureDate(3))
        cy.get(LECourseLessonPlayerPage.getCoursePlayerWarningBanner()).should('have.css', 'background-color', defaultThemeColors.default_warning_rgb)
