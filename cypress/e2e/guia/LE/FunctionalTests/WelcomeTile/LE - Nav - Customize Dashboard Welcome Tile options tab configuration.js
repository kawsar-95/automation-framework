...
describe('LE - Welcome Tile - Options Tab', function () {
    ...
    it('Verify Welcome Tile Height Customization', () => {
        //Edit Welcome Tile in the Tile container
        cy.get(LEManageTemplateTiles.getContainerByIndex(1)).within(() => {
            LEManageTemplateTiles.getTileEditBtnByTileName(dashboardDetails.body)
        })
        ...
        cy.get(LEManageTemplateTiles.getWelcomeTileHeightInput()).clear()
        cy.get(LEManageTemplateTiles.getWelcomeTileHeightInput()).type(dashboardDetails.height_for_test)
        
        cy.get(LEManageTemplateTiles.getWelcomeTileSavebutton()).contains('Save').click({force: true})

    })

    it('Verify Welcome Tile URL Customization', () => {
    ...
    it('Verify Welcome Tile can be viewed differently by mobile,tablet and desktop', () => {
        ...
        cy.get(LEManageTemplateTiles.getWelcomeTileMobileViewIcon()).should('be.visible')
        cy.get(LEManageTemplateTiles.getWelcomeTileMobileViewIcon()).click()

        // welcome tile Tablet view selection and visibility
        cy.get(LEManageTemplateTiles.getWelcomeTileTabletViewIcon()).should('be.visible')
        cy.get(LEManageTemplateTiles.getWelcomeTileTabletViewIcon()).click()
        
        // welcome tile Desktop view selection and visibility
        cy.get(LEManageTemplateTiles.getWelcomeTileDesktopViewIcon()).should('be.visible')
        cy.get(LEManageTemplateTiles.getWelcomeTileDesktopViewIcon()).click()

        //save details with all the customizations
        cy.get(LEManageTemplateTiles.getWelcomeTileSavebutton()).contains('Save').click({force: true})
