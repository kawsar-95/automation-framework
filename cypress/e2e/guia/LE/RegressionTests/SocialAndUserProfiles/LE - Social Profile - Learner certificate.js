...
describe('C1804 - AUT-427 - GUIA-Story - Acceptance Test: NLE-1872 - A learner has a social profile view - certificates and total', () => {
    it('Create a new learner and enroll that learner with certificates course', () => {
        ...
        //create a new course with certificates
        cy.createCourse('Online Course')
        cy.get(ARAddMoreCourseSettingsModule.getCourseSettingsByNameBtn('Completion')).dblclick()
        //Enable certificate with expiry
        ARCourseSettingsCompletionModule.getToggleByNameThenClick('Certificate')
        //Clicking on URL of the certificate
        ARCourseSettingsCompletionModule.getCertificateTypeRadioButton("Url").click()

        //Removing the chapter from the course so that user dont have to complete the course 
        cy.get(ARCourseSettingsCompletionModule.getChaptersRemoveBtn()).click()

        //Deleting from the prompt modal 
    ...
    it('Log in to the LE side and go to course', () => {
        ...
        cy.get(LEResourcesPage.getCourseCardName()).should('contain', ocDetails.courseName).click()
