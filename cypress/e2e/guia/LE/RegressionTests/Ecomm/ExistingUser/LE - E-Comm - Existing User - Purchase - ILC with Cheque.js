...
describe('LE - E-Comm - Existing User - Purchase - ILC with Cheque', function(){
...
describe('LE - E-Comm - Existing User - Purchase - ILC with Cheque - Approve Purchase', function(){
    ...
    it('Admin - Find transaction and approve', () => { 
        ...
        LEDashboardPage.getVShortWait()
        //Approve the transaction - medium wait to ensure approval completes
        cy.get(ARDashboardPage.getA5AddEditMenuActionsByIndex(4)).click()
        LEDashboardPage.getMediumWait()
    })
})

describe('LE - E-Comm - Existing User - Purchase - ILC with Cheque - Verify Purchase', function(){
    ...
    it('Verify newly purchased ILC course can be started & user has been enrolled in correct session', () => { 
        ...
        /* Commenting this out for now as this sometimes takes a very long time to load and fails the test
           This could be re-added once intercepts have been fixed.
        LEDashboardPage.getMediumWait() //Wait for session details to load - this is sometimes slower
        cy.get(LECourseDetailsILCModule.getEnrolledSessionNameTxt()).should('contain', 'FUTURE SESSION')
        */
    })
})