...
const CED_CURR_NAME = 'GUIA - VARIABLE - CURR - ' + timestamp;
...
describe('LE - E-Comm - Existing User - Purchase - CURR with Variable Pricing - Create Course', function(){

    it('Create Curriculum with Variable Price Rules & Publish', () => {
        cy.loginAdmin(users.sysAdmin.ADMIN_SYS_01_USERNAME, users.sysAdmin.ADMIN_SYS_01_PASSWORD)
        arCoursesPage.getAddCurricula()
        cy.get(ARCURRAddEditPage.getElementByAriaLabelAttribute(ARCURRAddEditPage.getGeneralTitleTxtF())).clear().type(CED_CURR_NAME)
        cy.get(ARCURRAddEditPage.getGeneralStatusToggleContainer() + ARCURRAddEditPage.getToggleDisabled()).click()
        ...
        //Enable E-Commerce
        cy.get(ARCURRAddEditPage.getElementByDataNameAttribute(ARCourseSettingsEnrollmentRulesModule.getEnableEcommToggleContainer()) + ' '
             + ARCURRAddEditPage.getToggleEnabled()).click()
             ARCURRAddEditPage.getVShortWait()
        //Allow Public Purchase
        cy.get(ARCURRAddEditPage.getElementByDataNameAttribute(ARCourseSettingsEnrollmentRulesModule.getEnablePublicPurchaseContainer()) + ' '
             + ARCURRAddEditPage.getToggleEnabled()).click()
...
describe('LE - E-Comm - Existing User - Purchase - CURR with Variable Pricing', function(){
    ...
    after(function() {
        ...
        cy.deleteCourse(courseID, 'curricula')
    ...
    for (let i = 0; i < userNames.length; i++) {
        it(`Verify Variable Price for Learner in ${departmentNames[i]} Department`, () => {
            ...
            //Add Course to Cart
            LEFilterMenu.SearchForCourseByName(CED_CURR_NAME)
            LEDashboardPage.getMediumWait()
            LEDashboardPage.getCourseCardBtnThenClick(CED_CURR_NAME)
            LEDashboardPage.getMediumWait()
