...
const CED_OC_NAME = 'GUIA - VARIABLE - OC - ' + timestamp;
...
describe('LE - E-Comm - Existing User - Purchase - OC with Variable Pricing - Create Course', function(){

    it('Create OC Course with Variable Price Rules & Publish', () => {
        cy.loginAdmin(users.sysAdmin.ADMIN_SYS_01_USERNAME, users.sysAdmin.ADMIN_SYS_01_PASSWORD)
        arCoursesPage.getAddOnlineCourse()
        cy.get(AROCAddEditPage.getGeneralTitleTxtF()).clear().type(CED_OC_NAME)
        ...
        //Enable E-Commerce
        cy.get(AROCAddEditPage.getElementByDataNameAttribute(ARCourseSettingsEnrollmentRulesModule.getEnableEcommToggleContainer()) + ' '
             + AROCAddEditPage.getToggleEnabled()).click()
        AROCAddEditPage.getVShortWait()
        //Allow Public Purchase
        cy.get(AROCAddEditPage.getElementByDataNameAttribute(ARCourseSettingsEnrollmentRulesModule.getEnablePublicPurchaseContainer()) + ' '
             + AROCAddEditPage.getToggleEnabled()).click()
...
describe('LE - E-Comm - Existing User - Purchase - OC with Variable Pricing', function(){
