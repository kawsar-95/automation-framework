...
describe('AUT-390 - T98581 - GUIA-Story - NLE-3849 - Course Player - Overview tab Outcomes - Verify Confirming to Leave Lesson while course upload', () => {
    ...
    before('Create competencies, Question Bank and Enable NextGen LE', () => {
        ...
        cy.get(ARQuestionBanksPage.getGridTable(), {timeout: 30000}).should('be.visible')
    ...
    it('Confirming to Leave Lesson while course upload', () => {
        ...
        cy.get(ARSelectLearningObjectModal.getSaveBtn()).should('have.attr','aria-disabled','false')
        ...
        // Enroll User
        AREnrollUsersPage.getEnrollUserByCourseAndUsername([ocDetails.courseName4], [userDetails.username4])
        // Vist to the usres report page
        ARDashboardPage.getUsersReport()
        ARDashboardPage.AddFilter('Username', 'Contains', userDetails.username4)
        cy.get(ARDashboardPage.getWaitSpinner(), { timeout: 60000 }).should('not.exist')
        ...
        LEDashboardPage.getCourseCardBtnThenClick(ocDetails.courseName4)
        cy.get(ARDashboardPage.getWaitSpinner(), {timeout: 60000}).should('not.exist')
        cy.get(LEDashboardPage.getCollapseIcon()).click()
        cy.get(ARDashboardPage.getWaitSpinner(), {timeout: 60000}).should('not.exist')

        cy.get(LEDashboardPage.getChevronIcon(), {timeout: 60000}).should('be.visible')
        cy.get(LEDashboardPage.getChevronIcon(), {timeout: 60000}).click()

        cy.get(LEDashboardPage.getCompetencyHoverSec()).should('contain','Find Related Courses').click()       
        cy.get(LEDashboardPage.getRedirectModelOptionbtn()).contains('Leave Upload').should('be.visible').click()
