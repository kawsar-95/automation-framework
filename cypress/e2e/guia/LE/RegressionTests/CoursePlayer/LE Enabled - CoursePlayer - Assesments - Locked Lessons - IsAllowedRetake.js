...
describe('T98589 - AUT-387 - GUIA-Auto - NLE-3470 : Course Player - Locked Lessons - IsAllowedRetake', function () {
    ...
    it('Create Online Course with an Assessment and Video Lesson', () =>{
        ...
        arDashboardPage.getMediumWait()
        cy.get('[data-name="name"]').contains("Assessment Name").should('exist')
    ...
    it('Verify lesson course completition cards after successful Assessment',()=>{
    ...
    it('Verify lesson course completition cards after failing Assessment',()=>{
        cy.viewport(1025,660)
    ...
    //E-signature "completed" card   
    it('Create Online Course with an E-signature (Authentication) and Video Lesson', () =>{
        //Sign into admin side as sys admin, navigate to Courses
        cy.apiLoginWithSession(users.sysAdmin.admin_sys_01_username, users.sysAdmin.admin_sys_01_password, '/admin')
        cy.get(ARDashboardPage.getElementByAriaLabelAttribute(ARDashboardPage.getARLeftMenuByLabel('Courses'))).click()
        cy.wrap(ARDashboardPage.getMenuItemOptionByName('Courses'))
        ...
        LEDashboardPage.getShortWait()
        ARCourseSettingsEnrollmentRulesModule.getAllowSelfEnrollmentRadioBtn('All Learners')
        //Add E-signature lesson object to the course 
        cy.get(AROCAddEditPage.getAddLearningObjectBtn()).should('have.attr', 'aria-disabled', 'false').click()
        ARSelectLearningObjectModal.getObjectTypeByName('E-Signature')
        cy.get(ARSelectLearningObjectModal.getNextBtn()).click()
        arDashboardPage.getLongWait()
        ARAddObjectLessonModal.getTxtFByName('Name',eSignature.eSignatureName)  
        cy.get(ARAddObjectLessonModal.getQuestionSavebutton()).contains("Save").click()   
        arDashboardPage.getMediumWait()
        //Add Video File Object Lesson Desktop Iframe
        cy.get(AROCAddEditPage.getAddLearningObjectBtn()).should('have.attr', 'aria-disabled', 'false').click()
        ...
        cy.get(ARDashboardPage.getToastSuccessMsg(), {timeout: 10000}).should('contain', 'Course successfully published')
        //Enroll User
        arEnrollUsersPage.getEnrollUserByCourseAndUsername([ocDetails.courseName2], [userDetails.username3])
        LEDashboardPage.getShortWait()
    })

    it('Verify lesson course completition cards after successful E-signature',()=>{
        cy.viewport(1025,660)
        ...
        LEDashboardPage.getVShortWait()
        //The Lesson Complete card will be displayed, with the Status as Complete 
        cy.get(LEDashboardPage.getUsernameTxtF()).type(userDetails.username3)
        cy.get(LEDashboardPage.getEsignaturePasswordTxtF()).type(userDetails.validPassword)
        ...
        cy.get(LECourseDetailsOCModule.getStartBtn()).click()
        LEDashboardPage.getMediumWait()
        LECourseLessonPlayerPage.getStartLessonByName(eSignature.eSignatureName)  
        LEDashboardPage.getLongWait() 
        cy.get(LECourseLessonPlayerPage.getEarlyLeavePromptHeader()).should('be.visible').and('contain','Lesson Complete')
        cy.get(LECourseLessonPlayerPage.getCoursePlayerCardModuleMessage()).eq(1).should('contain',`Looks like you’ve already completed this lesson.`)
        cy.get(LECourseLessonPlayerPage.getCoursePlayerStatusModule()).eq(0).should('be.visible').and('contain','Status')
    ...
    //OJT passed, Display All Results   - this is pending OJT addition to the Automation
    //OJT failed, Display All Results; if only lesson, results will be displayed after fix in NLE-4266
