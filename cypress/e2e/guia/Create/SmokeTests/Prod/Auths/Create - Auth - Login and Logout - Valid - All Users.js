/**
 * Tests for the Create Login and Logout process for all users (valid cases only) in Prod
 * Verifying the user reaches the correct page and  can log in
 * Verifying that the correct user is logged in
 * 
    Covers: 
    QAAUT-2889 - GUIA - Create - Stand Alone Prod - Login and Logout - Valid
 */
        ...
        for (let i = 0; i < userName.length; i++) {
            switch (userName[i]) {
                case users.createReviewer.create_type_reviewer:
                    cy.createAdminProd(users.createReviewer.create_reviewer_username, users.createReviewer.create_reviewer_password);
                ...
                case users.createAuthor.create_type_author:
                    cy.createAdminProd(users.createAuthor.create_author_username, users.createAuthor.create_author_password);
                    break;
                case users.createDesigner.create_type_designer:
                    cy.createAdminProd(users.createDesigner.create_designer_username, users.createDesigner.create_designer_password);
                    break;
                ...
                case users.createProducer.create_type_producer:
                    cy.createAdminProd(users.createProducer.create_producer_username, users.createProducer.create_producer_password);
                    break;
                case users.createPublisher.create_type_publisher:
                    cy.createAdminProd(users.createPublisher.create_publisher_username, users.createPublisher.create_publisher_password);
                    break;
